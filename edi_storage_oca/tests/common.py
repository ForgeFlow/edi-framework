...
class TestEDIStorageBase(EDIBackendCommonTestCase):
    ...
    def _mock_fs_storage_list_files(self, mocked_paths):
        ...
        return mock.patch(FS_STORAGE_MOCK_PATH + ".list_files", mocked)

    def _mock_fs_storage_find_files(self, result):
        def _result(self, pattern, relative_path=None, **kw):
            return result

        return mock.patch(FS_STORAGE_MOCK_PATH + ".find_files", _result)
    ...
    def _test_result(
        ...
        for state in state_paths:
            ...
            self.assertIn(path, self._fs_storage_calls)
    ...
    def _test_send(self, record, mocked_paths=None):
        with self._mock_fs_storage_add():
            if mocked_paths:
                with self._mock_fs_storage_get(mocked_paths):
                    self.backend.exchange_send(record)
            else:
                self.backend.exchange_send(record)

    def _test_run_cron(self, mocked_paths, skip_sent=True):
        with self._mock_fs_storage_add():
            with self._mock_fs_storage_get(mocked_paths):
                self.backend._cron_check_output_exchange_sync(skip_sent=skip_sent)

    def _test_run_cron_pending_input(self, mocked_paths):
        with self._mock_fs_storage_add():
            with self._mock_fs_storage_list_files(mocked_paths):
